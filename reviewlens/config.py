...
# หน่วยเงินที่เป็นตัวอักษรต้องมี \b นำหน้าเสมอ
# ไม่งั้น "rs" จะไปแมตช์ท้ายคำอื่น ทำให้ "service centers 2" กลายเป็นราคา 2 บาท
# และ "upto 4 years4" กลายเป็น 4 (เจอจริง 7 เคสจาก 300 รีวิวแรกของชุดข้อมูล)
PRICE_PATTERN = re.compile(
    # รูปแบบนำหน้าใช้ \b เฉพาะด้านหน้า เพื่อให้ "rs38000" ที่เขียนติดกันยังจับได้
    r"(?:฿|₹|\b(?:rs|inr|thb)\.?)\s*([\d,]+(?:\.\d+)?)"            # ฿1,290 / Rs. 6195
    r"|([\d,]+(?:\.\d+)?)\s*(?:บาท|฿|₹|\b(?:thb|rs|inr|rupees?)\b)",  # 890 บาท / 6195 rs
    re.I,
)
